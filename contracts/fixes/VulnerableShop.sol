...
contract VulnerableShop is Ownable {
    ...
    */
    ...
    function newSale(string calldata title, string calldata description, uint256 price) external {
        // first check msg.sender token balance in order to stake funds
        require(token.balance(msg.sender) >= price, "itemId does not exist");
    ...
    function reimburse(uint256 itemId) external onlyOwner {
        ...
        */
        ...
        (bool result, ) = token.safeTransfer(msg.sender, amount);
        require(result, "Token safe transfer failed");
